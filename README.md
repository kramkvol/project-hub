# Project Hub

This repository is a central hub containing links to my projects from different stages of my education and personal development.

## Summary of Demonstrated Skills
- **Skills Developed through Academic & Personal Projects:**
    - **Programming Languages:** `Java` `C++` `Kotlin` `Python`
    - **Core Concepts:** `OOP` `Software Design Patterns`
    - **Databases & SQL:** `PostgreSQL` `MySQL` `Microsoft SQL Server`

    - **Java Ecosystem:**
        - **Frameworks & Libraries:** `Spring Boot` `JavaFX`
        - **Testing:** `JUnit` `Mockito`
        - **Persistence & ORM:** `JPA (Jakarta Persistence)` `Hibernate`
        - **Architecture & Practices:** `CRUD` `DAO Layer` `Layered Architecture`

    - **Android Development:** `Android SDK` `Jetpack Compose` `ADB`
    - **Python Ecosystem:** `Pandas` `Jupyter Notebook`
    - **Web Development:** `HTML5` `CSS3` `JavaScript (ES6)` `PHP`
    - **C# / .NET:** `WinForms`
    - **Low-code Tools:** `Microsoft PowerApps` `Bubble`

- **Tools & Environment:**
    - **Version Control:** `Git` `GitHub` `GitLab`
    - **IDEs:** `IntelliJ IDEA` `CLion` `VS Code` `Visual Studio` `Android Studio`
    - **Build Tools:** `Maven` `Gradle` `CMake`
    - **Python Tooling:** `pip` `venv`
    - **Database Tools:** `MySQL Workbench` `SQL Server Management Studio (SSMS)`

- **Other Tools**
    - `Adobe Photoshop` `Adobe Illustrator` `Adobe Animate` `Adobe Premiere Pro` `CorelDRAW`
    - `Figma`
    - `3ds Max` `AutoCAD`
    - `Microsoft Office` (`Word` `PowerPoint` `Excel` `Access`)
    - `Camunda`
    - `PowerBI`


## Table of Contents
Main content:
- [Java Experimental projects Table](#experimental-java)
- [Java CTU projects projects Table](#ctu-projects-java)

- [C# Experimental projects Table](#experimental-cmp-csh)
- [C++ CTU projects projects Table](#ctu-projects-cpp)

Other content:
- [CTU Web projects Table](#ctu-projects-web)
- [CTU Other projects Table](#ctu-projects-other)

## Main Contents
- **Public** Repository with educational projects in `Java`: https://github.com/kramkvol/java_roadmap
- **Private** college projects archive (`C++` / `C#`): https://github.com/kramkvol/projects_before_university

<a id="experimental-java"></a>
### Java Experimental
| Project             | Focus                                                    | Status      | Link                                            |
|---------------------|----------------------------------------------------------|-------------|-------------------------------------------------|
| Coffee Order System | `Design Patterns` (Factory, Strategy, Observer, Builder) | In progress | [Repo](https://github.com/kramkvol/patterns_p2) |

<a id="ctu-projects-java"></a>
### Java CTU projects 
| Course                                                 | What I practiced in the course                                          | Comment                              | Repository status | Repository Link                                                          |
|--------------------------------------------------------|-------------------------------------------------------------------------|--------------------------------------|-------------------|--------------------------------------------------------------------------|
| `B0B36PJV` `BI-PJV`<br/> `B6B36TS1 (Software Testing)` | `JavaFX` `Multithreading` <br/> `Spring Boot` `JUnit` `Mockito`         | Game "Sudoku"                        | `public`          | https://github.com/kramkvol/PJV<br/>https://github.com/kramkvol/B6B36TS1 |
| `B6B36OMO Object-oriented design and Modeling`         | `Stream API` `Software Design Patterns`                                 | Team project "Smart home"            | `public`          | https://github.com/kramkvol/B6B36OMO                                     |
| `B0B36DBS Database Systems`                            | `Jakarta Persistence (JPA)` `Hibernate` `PostgreSQL` `DAO layer` `CRUD` | -                                    | -                 | -                                                                        |
| `B0B36TPA Creation of business applications`           | `Java` `Camunda` `Bubble` `Microsoft PowerApps` `MySQL` `Ngrok`         | LightGrow Translation Agency Process | `private`         | https://github.com/kramkvol/B0B36TPA                                     |

<a id="experimental-cmp-csh"></a>
### C# Experimental Projects
| Project                          | Focus                                              | Status | Link                                            |
|----------------------------------|----------------------------------------------------|--------|-------------------------------------------------|
| Encryption Algorithms (WinForms) | `Design Patterns` `Strategy` `Decorator` `Command` | Public | [Repo](https://github.com/kramkvol/patterns_p1) |

<a id="ctu-projects-cpp"></a>
### C++ CTU projects
| Course   | Project                  | Topics                         | Status | Link                                         |
|----------|--------------------------|--------------------------------|--------|----------------------------------------------|
| B6B36PCC | Winston Cipher Algorithm | `CLI` `Multithreading` `Files` | Public | [Repo](https://github.com/kramkvol/B6B36PCC) |

## Other Contents
<a id="ctu-projects-web"></a>
### CTU Web projects
| Subject                                      | What I practiced in the course                                                                        | Repository status | Repository Link                      |
|----------------------------------------------|-------------------------------------------------------------------------------------------------------|-------------------|--------------------------------------|
| `B6B39ZWA Foundations of Web Application`    | `HTML5` `JavaScript` `CSS3` `PHP` `FileZilla`                                                         | `public`          | https://github.com/kramkvol/B6B39ZWA |
| `B0B39KAJ Client applications in JavaScript` | `HTML5` `CSS3` `JavaScript (ES6)` `Service Workers` `localStorage` `Drag & Drop API` `FileReader API` | `private`         | https://github.com/kramkvol/B0B39KAJ |

<a id="ctu-projects-other"></a>
### CTU Others projects
| Subject                                                   | What I practiced in the course                                                            | Comment                                     | Repository status | Repository Link                      |
|-----------------------------------------------------------|-------------------------------------------------------------------------------------------|---------------------------------------------|-------------------|--------------------------------------|
| `B0B36ZAL Introduction to Programming`                    | `Python`                                                                                  | All topics are in readme.md                 | `public`          | https://github.com/kramkvol/B0B36ZAL |
| `B6B39ZAN Basic Android development`                      | `Kotlin`                                                                                  | Application for managing notes about books. | `private`         | https://github.com/kramkvol/B6B39ZAN |
| `B6B16ZDA Basics of data analysis`                        | `Python` `Pandas` `Jupyter Notebook` `PowerBI`                                            |                                             | `private`         | https://github.com/kramkvol/B6B16ZDA |
| `B6B36SMP Analysis and Modeling of Software Requirements` | `Enterprise Architect` `UML modeling` `Scrum`<br/> `Requirements analysis and validation` | Team Project                                | `private`         | https://github.com/kramkvol/B6B36SMP |
